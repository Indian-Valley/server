...
async function getCustomer(req, res) {
    ...
    if (!customer_uuid) {
        return res.status(400).json({ success: false, error: 'No customer id provided' })
    }
...
async function updateCustomerDetails(req, res) {
    ...
    if (error) {
        console.error(error)
        return res.status(400).json({ success: false, error: 'Failed to update customer details', fullError: error })
    }

    return res.status(200).json({ success: true, data })
}

async function getCustomerCurrentOrders(req, res) {
    const { customer_id } = req.params
...
async function deleteCustomer(req, res) {
    ...
    if (resCustomerTable?.error) {
        ...
        return res.status(400).json({ success: false, error: 'Failed to delete customer', fullError: resCustomerTable?.error })
    }
    ...
    if (error) {
        ...
        return res.status(400).json({ success: false, error: 'Failed to delete customer', fullError: error })
    }
...
module.exports = {
    getCustomer, updateCustomerDetails, getCustomerPreviousOrders, deleteCustomer, getCustomerCurrentOrders
}